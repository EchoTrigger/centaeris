...
Persistent-format changes must also pass the retained [upgrade acceptance](UpgradeAcceptance.md).
Use fixtures generated by the released version, not only fixtures written by the
new serializer. These Rust tests run in the existing workspace test gate.
