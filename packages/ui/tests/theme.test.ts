...
function boot(saved: string | null, dark: boolean, blocked = false) {
  ...
  const meta = { content: "", name: "theme-color" };
  const media = { matches: dark, addEventListener: (_: string, fn: () => void) => listeners.set("system", fn) };
  const scope = { document: { documentElement: { dataset, style: {} }, querySelector: () => meta }, matchMedia: () => media,
    localStorage: { getItem: () => { if (blocked) throw Error("blocked"); return saved; } },
    Event, dispatchEvent: (event: Event) => listeners.get(event.type)?.(),
  ...
  return { dataset, media, listeners, meta, root: scope.document.documentElement };
}
...
test("page theme exposes the matching native titlebar color", () => {
 const app = boot("dark", false);
 expect(app.meta.content).toBe("#242424");
 app.listeners.get("storage")?.({ key: "centaeris:theme:v1", newValue: "light" });
 expect(app.meta.content).toBe("#f5f5f4");
});
