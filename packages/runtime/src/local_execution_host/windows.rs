...
fn validate_policy(policy: &ExecutionPolicy) -> Result<(), ExecutionError> {
    ...
    Err(ExecutionError::PolicyUnavailable {
        reason: "Windows Git Bash host execution supports only the publicInternet network policy"
            .to_string(),
    })
}
...
pub(super) fn prepare_command(
    ...
    validate_policy(policy)?;
    let program = validate_git_bash(program).ok_or_else(|| ExecutionError::Denied {
        reason: format!(
    ...
    request.validate().map_err(ExecutionError::Io)?;
    let input = serde_json::to_vec(&request).map_err(|error| {
        ExecutionError::Io(format!("encode Windows host launch failed: {error}"))
    })?;
...
pub(super) fn resolve_git_bash_path(
    ...
    if let Some(path) = explicit_bash_path {
        return validate_git_bash(path.as_path()).ok_or_else(|| ExecutionError::HostUnavailable {
            reason: format!(
...
mod tests {
    ...
    #[test]
    fn host_launch_request_rejects_non_public_internet_network_policy() {
        let policy = ExecutionPolicy::workspace_write_no_network(std::path::Path::new("."));
        match prepare_command(
            Path::new("centaeris-runtime.exe"),
            ...
            Ok(_) => panic!("disabled network policy must be rejected before dispatch"),
        }
    }
}
